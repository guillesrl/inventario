  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-5">
        <h2 className="text-base font-semibold text-gray-700 dark:text-gray-200 mb-4">Últimas ventas</h2>
        {recentSales.length === 0 ? (
          <p className="text-gray-400 dark:text-gray-500 text-sm">Sin ventas registradas aún.</p>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-700">
              {recentSales.map((sale) => (
                <tr key={sale.id} className="text-gray-900 dark:text-gray-100">
                  <td className="py-2">{sale.product.name}</td>
                  <td className="py-2">{sale.quantity}</td>
                  <td className="py-2">${sale.total.toFixed(2)}</td>
                  <td className="py-2 text-gray-400 dark:text-gray-500">{sale.createdAt.toLocaleDateString('es')}</td>
                </tr>
              ))}
            </tbody>
          </table>
